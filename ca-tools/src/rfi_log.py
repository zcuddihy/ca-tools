...
class RFI_Log:
    def __init__(self, new_RFI: dict, project_obj: object):
    ...
    def save(self):
        ...
        current_submittals = self.wb.sheets[sheet_name].range(f"A8:A{lastRow-1}").value

        for RFI in self.new_RFI:
            if float(RFI) in current_submittals:
                continue
            else:
                rfi_description = self.new_RFI[RFI]["Description"]
                rfi_url = self.new_RFI[RFI]["URL"]
